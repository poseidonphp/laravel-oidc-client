...
if (!class_exists('\phpseclib3\Crypt\RSA') && !class_exists('Crypt_RSA')) {
...
class OpenIDConnect
{
    ...
    const CREATE_IF_NOT_EXIST = 1;
    const SYNC_USER_FROM_CLAIMS = 2;
    const CREATE_AND_SYNC_USER = 3;
     ...
     * @var string
     */
    public string | null $intendedUrl;
     ...
     */
    public function authenticate(int $userAction = 0): bool {
        ...
        if (isset($_REQUEST['code'])) {
            ...
            if ($this->verifyJWTclaims($claims, $token_json->access_token)) {
                ...
                $this->unsetNonce();
                $this->intendedUrl = $this->getSessionKey('intendedUrl');
        ...
        if($userAction === self::SYNC_USER_FROM_CLAIMS) {
            try {
                if(!config('oidc.user_model')) throw new \Exception('User model not defined in config');
                $user = config('oidc.user_model')::where(config('oidc.username_property_in_model'), '=', $this->getVerifiedClaims(config('oidc.username_property_in_claim')))->firstOrFail();
        ...
        if ($userAction === self::CREATE_IF_NOT_EXIST || $userAction === self::CREATE_AND_SYNC_USER) {
            try {
                if(!config('oidc.user_model')) throw new \Exception('User model not defined in config');
                $user = config('oidc.user_model')::where(config('oidc.username_property_in_model'), '=', $this->getVerifiedClaims(config('oidc.username_property_in_claim')))->firstOrFail();
            } catch (ModelNotFoundException $exception) {
...
//        $this->requestAuthorization(); //commented out 6-20-22
        return false;
...
//            dd($well_known_config_url);
    ...
    /**
     * @param string $url Sets the intended URL to use later after authentication
     */
    public function setIntendedUrl($url)
    {
        $this->intendedUrl = $url;
        $this->setSessionKey('intendedUrl', $url);
    }
    ...
    protected function requestTokens($code) {
        ...
        // dd($this->fetchURL($token_endpoint, $token_params, $headers));
    ...
    private function verifyRSAJWTsignature($hashtype, $key, $payload, $signature, $signatureType) {
        ...
        */
        ...
        if(class_exists('Crypt_RSA', false)) {
          ...
          /** The following section was commented and updated by Dan **/
            $rsa = \phpseclib3\Crypt\PublicKeyLoader::load($public_key_xml); // added by dan
            $rsa2 = \phpseclib3\Crypt\RSA::load($rsa);
            $rsa2 = $rsa2->withPadding(\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
            // dd(['payload' => $payload, 'key' => $key, 'rsa' => $rsa, 'rsa2' => $rsa2]);
            // die($rsa2->toString('pkcs1')); // THIS WORKS TO VALIDATE!!!!
            // dd(["cert" => $rsa2->toString('pkcs1'), 'payload' => $payload, 'key' => $key]);
            return $rsa2->verify($payload, $signature);
            // $rsa = new \phpseclib3\Crypt\RSA();
            // $rsa->setHash($hashtype);
            // if ($signatureType === 'PSS') {
            //     $rsa->setMGFHash($hashtype);
            // }
            // $rsa->loadKey($public_key_xml, \phpseclib3\Crypt\RSA::PUBLIC_FORMAT_XML);
            // $rsa->signatureMode = $signatureType === 'PSS' ? \phpseclib3\Crypt\RSA::SIGNATURE_PSS : \phpseclib3\Crypt\RSA::SIGNATURE_PKCS1;
        }
    ...
    public function canVerifySignatures() {
        return class_exists('\phpseclib3\Crypt\RSA') || class_exists('Crypt_RSA');
    }
